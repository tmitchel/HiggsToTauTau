...
#ifndef INCLUDE_SF_FACTORY_H_
#define INCLUDE_SF_FACTORY_H_
...
class SF_factory {
 ...
 public:
    explicit SF_factory(std::string);
    virtual ~SF_factory() {}
...
SF_factory::SF_factory(std::string fname) {
    ...
    etaBinsH = static_cast<TH1D*>(fin->Get("etaBinsH"));
    ...
    for (int ibin = 0; ibin < nEtaBins; ibin++) {
        ...
        if (fin->GetListOfKeys()->Contains(data_name.c_str())) {
            eff_data[eta_label] = static_cast<TGraphAsymmErrors*>(fin->Get(data_name.c_str()));
        ...
        if (fin->GetListOfKeys()->Contains(mc_name.c_str())) {
            eff_mc[eta_label] = static_cast<TGraphAsymmErrors*>(fin->Get(mc_name.c_str()));
        ...
        if (eff_data[eta_label] != 0 && eff_mc[eta_label] != 0) {
            if (!checkBinning(eff_data[eta_label], eff_mc[eta_label])) std::cout << "ERROR in ScaleFactor::ScaleFactor(std::string)" << std::endl;
        }
    }
}
...
bool SF_factory::checkBinning(TGraphAsymmErrors* g1, TGraphAsymmErrors* g2) {
    if (g1->GetXaxis()->GetNbins() != g2->GetXaxis()->GetNbins()) {
        ...
        for (int ibin = 0; ibin < g1->GetXaxis()->GetNbins(); ibin++) {
            if ((g1->GetXaxis()->GetXbins())->GetArray()[ibin] != (g2->GetXaxis()->GetXbins())->GetArray()[ibin]) return false;
        }
    }
    return true;
}
