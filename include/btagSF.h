...
#ifndef INCLUDE_BTAGSF_H_
#define INCLUDE_BTAGSF_H_
...
int PromoteDemote(TH2F *h_btag_eff_b, TH2F *h_btag_eff_c, TH2F *h_btag_eff_oth, jet *bjet, int nbtag, int syst) {
    ...
    float beff = 1.0;
    if (bjet->getFlavor() == 5) {
        // b-jet (get bjet pT or 1 lower than max bin edge)
        auto effective_pt = std::min(bjet->getPt(), static_cast<Float_t>(h_btag_eff_b->GetXaxis()->GetBinLowEdge(h_btag_eff_b->GetNbinsX() + 1) - 1));
        beff == h_btag_eff_b->GetBinContent(effective_pt, h_btag_eff_b->GetYaxis()->FindBin(fabs(bjet->getEta())));

    } else if (bjet->getFlavor() == 4) {
        // c-jet
        auto effective_pt = std::min(bjet->getPt(), static_cast<Float_t>(h_btag_eff_c->GetXaxis()->GetBinLowEdge(h_btag_eff_c->GetNbinsX() + 1) - 1));
        beff == h_btag_eff_c->GetBinContent(effective_pt, h_btag_eff_c->GetYaxis()->FindBin(fabs(bjet->getEta())));

    } else {
        // light-jet
        auto effective_pt =
            std::min(bjet->getPt(), static_cast<Float_t>(h_btag_eff_oth->GetXaxis()->GetBinLowEdge(h_btag_eff_oth->GetNbinsX() + 1) - 1));
