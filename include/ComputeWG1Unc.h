...
static double g_sig0 = 30.117, g_sig1 = 12.928, g_sig_ge2 = 5.475, g_sig_ge1 = g_sig1 + g_sig_ge2, g_sig_tot = g_sig0 + g_sig_ge1,
              g_sig_vbfTopo = 0.630, g_sig_ge2noVBF = g_sig_ge2 - g_sig_vbfTopo, g_sig_ge1noVBF = g_sig_ge1 - g_sig_vbfTopo;
...
NumV blptw(int Njets30) {
    ...
    return {yieldUnc[jetBin] * normFact, resUnc[jetBin] * normFact, cut01Unc[jetBin] * normFact, cut12Unc[jetBin] * normFact};
}
...
double qm_t(double pT) { return interpol(pT, 160, 0.0, 500, 0.37); }
...
NumV qcd_ggF_uncert_wg1(int Njets30, double pT, int STXS) {
    ...
    double pTH_unc = 0.0;
    if (Njets30 == 1)
        pTH_unc = interpol(pT, 0, y1_1, x2_1, y2_1);
    else if (Njets30 >= 2)
        pTH_unc = interpol(pT, 0, y1_ge2, x2_ge2, y2_ge2);
...
NumV qcd_ggF_uncert_stxs(int Njets30, double pT, int STXS) {
    ...
    static double sig0_60 = 8.719, sig60_200 = 9.095, sig120_200 = 1.961, sig0_120 = sig0_60 + sig60_200 - sig120_200,
                  sig200_plus = 0.582;                                             // 0.121 (-) 0.151*0.582
    ...
    if (Njets30 >= 1) {
        if (pT < 60)
            dsig60 = -Dsig60_200 / sig0_60;  // -17.2%
        else if (pT < 200)
            dsig60 = Dsig60_200 / sig60_200;  // +16.0%

        if (pT < 120)
            dsig120 = -Dsig120_200 / sig0_120;  //  -2.6%
        else if (pT < 200)
            dsig120 = Dsig120_200 / sig120_200;  // +20.8%
