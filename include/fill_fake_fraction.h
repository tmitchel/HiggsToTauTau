...
#ifndef INCLUDE_FILL_FAKE_FRACTION_H_
#define INCLUDE_FILL_FAKE_FRACTION_H_
...
FakeFractions::FakeFractions(std::string channel_prefix, std::string year, std::string suffix)
    : fout(new TFile(("Output/fake_fractions/" + channel_prefix + year + "_" + suffix + ".root").c_str(), "recreate")),
      ...
      categories{channel_prefix + "_inclusive", channel_prefix + "_0jet", channel_prefix + "_boosted", channel_prefix + "_vbf"} {
    for (auto cat : categories) {
        data.push_back(
            new TH2F(("data_" + cat).c_str(), ("data_" + cat).c_str(), mvis_bins.size() - 1, &mvis_bins[0], njets_bins.size() - 1, &njets_bins[0]));
        frac_w.push_back(new TH2F(("frac_w_" + cat).c_str(), ("frac_w_" + cat).c_str(), mvis_bins.size() - 1, &mvis_bins[0], njets_bins.size() - 1,
                                  &njets_bins[0]));
        frac_tt.push_back(new TH2F(("frac_tt_" + cat).c_str(), ("frac_tt_" + cat).c_str(), mvis_bins.size() - 1, &mvis_bins[0], njets_bins.size() - 1,
                                   &njets_bins[0]));
        frac_real.push_back(new TH2F(("frac_real_" + cat).c_str(), ("frac_real_" + cat).c_str(), mvis_bins.size() - 1, &mvis_bins[0],
                                     njets_bins.size() - 1, &njets_bins[0]));
        frac_qcd.push_back(new TH2F(("frac_qcd_" + cat).c_str(), ("frac_qcd_" + cat).c_str(), mvis_bins.size() - 1, &mvis_bins[0],
                                    njets_bins.size() - 1, &njets_bins[0]));
    }
...
#endif  // INCLUDE_FILL_FAKE_FRACTION_H_
